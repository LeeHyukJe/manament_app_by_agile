...
// your registration.js file so you can use a single integrity string and serve
...
```javascript
ssri.fromStream(fs.createReadStream('registration.js'), {
  algorithms: ['sha1', 'sha512']
}).then(integrity => {
  return ssri.checkStream(fs.createReadStream('registration.js'), integrity)
}) // succeeds
```
...
```javascript
const data = fs.readFileSync('registration.js')
...
```javascript
const integrity = ssri.fromData(fs.readFileSync('registration.js'))

ssri.checkStream(
  fs.createReadStream('registration.js'),
  integrity
)
// ->
// Promise<{
